...
const ConfigScreen = ({ onGenerate, isLoading }) => {
    ...
    return (
        <div className="min-h-screen bg-stone-950 p-4 sm:p-8 flex items-center justify-center font-serif">
            <div className="w-full max-w-5xl bg-neutral-200 p-8 rounded-lg shadow-2xl border-4 border-stone-500 relative">                <img src="https://r.resimlink.com/ukai9-Hct.png" alt="Top Secret Stamp" className="absolute top-4 right-4 w-24 h-24 opacity-80 -rotate-12 pointer-events-none" />                <div className="text-center mb-6">                    <h1 className="text-6xl font-bold text-stone-900" style={{fontFamily: "'Cormorant Garamond', serif"}}>Dava Jeneratörü</h1>                    <p className="text-stone-700 mt-2 text-lg">Bir sonraki gizemli maceranızı burada tasarlayın.</p>                </div>                                <form onSubmit={(e) => { e.preventDefault(); onGenerate(settings); }}>                    <div className="grid md:grid-cols-2 gap-8 mb-6">                        <FormCategory title="Hikaye Temelleri">                             <button type="button" onClick={handleRandomizeAll} disabled={isLoading || isRolling} className="flex items-center justify-center gap-2 w-full mb-4 px-4 py-2 bg-amber-500 border border-amber-600 rounded-lg text-white hover:bg-amber-400 transition-all duration-300 disabled:opacity-50 shadow-md">                                <BrainCircuit className={`w-5 h-5 ${isRolling ? 'animate-spin' : ''}`} />                                <span className="font-semibold">Yeni Fikirler Getir</span>                            </button>                                                        {storyFormFields.map(({label, name, placeholder}) => (                                <div key={name} className="mb-4">                                    <label className="font-semibold text-stone-700">{label}:</label>                                    <div className="flex items-center gap-2">                                        <input type="text" name={name} value={settings[name]} onChange={handleChange} placeholder={placeholder} className="w-full p-2 mt-1 bg-white/70 border border-stone-400 rounded-md shadow-sm"/>                                        <button type="button" onClick={() => handleRandomizeField(name)} disabled={isRefreshing === name || isLoading} className="p-2 text-stone-600 hover:text-amber-800 transition-colors disabled:opacity-50">                                            <RefreshCw className={`w-5 h-5 ${isRefreshing === name ? 'animate-spin' : ''}`} />                                        </button>                                    </div>                                </div>                            ))}                        </FormCategory>                                                <FormCategory title="Oyun Mekanikleri">                            <label className="font-semibold text-stone-700">Zorluk:</label>                             <select name="difficulty" value={settings.difficulty} onChange={handleChange} className="w-full p-2 mt-1 mb-4 bg-white/70 border border-stone-400 rounded-md shadow-sm">                                <option>Kolay</option><option>Normal</option><option>Zor</option><option>Uzman</option>                            </select>                            <label className="font-semibold text-stone-700">Oyun Süresi:</label>                            <select name="playTime" value={settings.playTime} onChange={handleChange} className="w-full p-2 mt-1 mb-4 bg-white/70 border border-stone-400 rounded-md shadow-sm">                                <option>1-2 Saat</option><option>3-4 Saat</option><option>5+ Saat</option>                            </select>                             <label className="font-semibold text-stone-700">Şüpheli Sayısı: <span className="text-amber-800 font-bold">{settings.suspectCount}</span></label>                             <input type="range" name="suspectCount" min="3" max={maxSuspects} value={settings.suspectCount} onChange={handleChange} className="w-full h-2 mt-2 bg-stone-300 rounded-lg appearance-none cursor-pointer accent-amber-800"/>                        </FormCategory>                    </div>                    <button type="submit" disabled={isLoading} className="w-full mt-4 bg-stone-800 hover:bg-stone-700 text-white font-bold py-4 px-4 rounded-lg transition duration-300 text-2xl shadow-xl disabled:bg-stone-500 disabled:cursor-wait" style={{fontFamily: "'Cormorant Garamond', serif"}}>                        {isLoading ? 'DAVA OLUŞTURULUYOR...' : 'DAVAYI OLUŞTUR'}                    </button>                </form>            </div>            <style>{`@import url('https://fonts.googleapis.com/css2?family=Cormorant+Garamond:wght@700&family=Tinos&display=swap'); body { font-family: 'Tinos', serif; }`}</style>        </div>    );};const LoadingScreen = () => (    <div className="min-h-screen bg-stone-950 flex flex-col items-center justify-center text-stone-200 font-serif" style={{backgroundImage: "url(https://www.transparenttextures.com/patterns/dark-wood.png)"}}>        <img src="https://r.resimlink.com/ukai9-Hct.png" alt="Çok Gizli Damgası" className="w-40 h-40 animate-pulse opacity-80"/>        <h2 className="text-4xl mt-6" style={{fontFamily: "'Cormorant Garamond', serif"}}>Dava Dosyası Hazırlanıyor...</h2>    </div>);const PasswordModal = ({ doc, onUnlock, onClose, setWrongPassword, wrongPassword }) => {    const [passwordInput, setPasswordInput] = useState('');        const handleSubmit = (e) => {        e.preventDefault();        if (doc.password && passwordInput.toLowerCase() === doc.password.toLowerCase()) {            onUnlock(doc.docId);        } else {            setWrongPassword(true);            setTimeout(() => setWrongPassword(false), 820); // match shake animation        }    };    return (        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 transition-opacity duration-300">            <div class={`bg-stone-800 p-8 rounded-lg shadow-2xl border-2 border-stone-600 w-full max-w-md text-center transform transition-all duration-300 ${wrongPassword ? 'animate-shake' : ''}`}>                <button onClick={onClose} className="absolute top-4 right-4 text-stone-400 hover:text-white"><X/></button>                <Lock className="w-20 h-20 text-amber-500 mx-auto mb-4"/>                <h2 className="text-3xl font-bold text-stone-100 mb-2" style={{fontFamily: "'Cormorant Garamond', serif"}}>KİLİTLİ DOSYA</h2>                <p className="text-stone-400 mb-6">Bu belgeye erişim için parola gereklidir.</p>                <form onSubmit={handleSubmit}>                    <input                         type="text" value={passwordInput} onChange={(e) => setPasswordInput(e.target.value)} autoFocus                        className="w-full p-3 text-center bg-stone-700 border-2 border-stone-600 rounded-md focus:outline-none focus:ring-2 focus:ring-amber-500 text-lg text-white tracking-widest"                        placeholder="••••••••"                    />                     {wrongPassword && <p className="text-red-500 mt-2">ERİŞİM REDDEDİLDİ</p>}                    <button type="submit" className="w-full mt-4 bg-amber-700 hover:bg-amber-600 text-white font-bold py-3 rounded-lg transition">Kilidi Aç</button>                </form>            </div>            <style>{`                @keyframes shake { 0%, 100% { transform: translateX(0); } 10%, 30%, 50%, 70%, 90% { transform: translateX(-10px); } 20%, 40%, 60%, 80% { transform: translateX(10px); } }                .animate-shake { animation: shake 0.82s cubic-bezier(.36,.07,.19,.97) both; }            `}</style>        </div>    );};const GameScreen = ({ gameData, onBack }) => {    const [selectedDocId, setSelectedDocId] = useState(null);    const [unlockedDocs, setUnlockedDocs] = useState(new Set());    const [docToUnlock, setDocToUnlock] = useState(null);    const [wrongPassword, setWrongPassword] = useState(false);        useEffect(() => {        const initialUnlocked = new Set(gameData.documents.filter(doc => !doc.locked).map(doc => doc.docId));        setUnlockedDocs(initialUnlocked);        if (gameData.documents && gameData.documents.length > 0) {            setSelectedDocId(gameData.documents[0].docId);        }    }, [gameData]);    const handleDocClick = (doc) => {        if (unlockedDocs.has(doc.docId)) {            setSelectedDocId(doc.docId);        } else {            setDocToUnlock(doc);        }    };        const handleUnlock = (docId) => {        setUnlockedDocs(prev => new Set(prev).add(docId));        setDocToUnlock(null);        setSelectedDocId(docId);    };    const selectedDocument = gameData.documents.find(d => d.docId === selectedDocId);    const interrogationReports = gameData.documents.filter(d => d.docType === 'Sorgu Raporu');    const otherMaterials = gameData.documents.filter(d => d.docType !== 'Sorgu Raporu');    return (        <>            {docToUnlock && <PasswordModal doc={docToUnlock} onClose={() => setDocToUnlock(null)} onUnlock={handleUnlock} setWrongPassword={setWrongPassword} wrongPassword={wrongPassword} />}            <div className="min-h-screen bg-stone-800 text-stone-800 p-4 font-serif flex flex-col" style={{backgroundImage: "url(https://www.transparenttextures.com/patterns/dark-wood.png)"}}>                <header className="flex-shrink-0 mb-4 bg-stone-300 p-4 rounded-lg shadow-lg border border-stone-400" style={{backgroundImage: "url(https://www.transparenttextures.com/patterns/old-paper.png)"}}>                    <div className="flex justify-between items-center">                        <h1 className="text-3xl font-bold text-stone-800" style={{fontFamily: "'Cormorant Garamond', serif"}}>{gameData.title}</h1>                        <button onClick={onBack} className="bg-stone-500 hover:bg-stone-400 text-white font-bold py-2 px-4 rounded-lg transition">Yeni Dava Başlat</button>                    </div>                    <p className="text-stone-700 mt-2 border-t border-stone-400 pt-2">{gameData.mainStory}</p>                </header>                <div className="flex-grow grid grid-cols-1 lg:grid-cols-4 gap-6 min-h-0">                    <div className="lg:col-span-1 bg-stone-300 backdrop-blur-sm rounded-lg p-4 border border-stone-400 flex flex-col shadow-lg" style={{backgroundImage: "url(https://www.transparenttextures.com/patterns/old-paper.png)"}}>                        <div className="flex-grow overflow-y-auto custom-scrollbar pr-2">                            {/* Sorgu Raporları Kategorisi */}                            <h2 className="text-xl font-bold text-stone-700 border-b-2 border-stone-400 pb-2 mb-3 flex items-center">                                <Users className="w-6 h-6 mr-3"/> Sorgu Raporları                            </h2>                            {interrogationReports.map(doc => (                                <div key={doc.docId} onClick={() => handleDocClick(doc)} className={`flex items-center p-3 mb-2 rounded-lg cursor-pointer transition border-2 ${selectedDocId === doc.docId ? 'bg-amber-800/20 border-amber-800' : 'border-transparent hover:bg-stone-400/30'}`}>                                    <div className="w-10 flex-shrink-0 flex items-center justify-center mr-3">                                        {unlockedDocs.has(doc.docId) ? <Icon name={doc.docType} type="doc" className="w-7 h-7 text-stone-700"/> : <Lock className="w-6 h-6 text-stone-500"/>}                                    </div>                                    <div><h3 className="font-bold text-stone-800 leading-tight">{doc.title}</h3></div>                                </div>                            ))}                            {/* Kanıt ve Belgeler Kategorisi */}                            <h2 className="text-xl font-bold text-stone-700 border-b-2 border-stone-400 pb-2 mb-3 mt-6 flex items-center">                                <FolderKanban className="w-6 h-6 mr-3"/> Kanıt & Belgeler                            </h2>                             {otherMaterials.map(doc => (                                <div key={doc.docId} onClick={() => handleDocClick(doc)} className={`flex items-center p-3 mb-2 rounded-lg cursor-pointer transition border-2 ${selectedDocId === doc.docId ? 'bg-amber-800/20 border-amber-800' : 'border-transparent hover:bg-stone-400/30'}`}>                                    <div className="w-10 flex-shrink-0 flex items-center justify-center mr-3">                                        {unlockedDocs.has(doc.docId) ? <Icon name={doc.docType} type="doc" className="w-7 h-7 text-stone-700"/> : <Lock className="w-6 h-6 text-stone-500"/>}                                    </div>                                    <div>                                        <h3 className="font-bold text-stone-800 leading-tight">{doc.title}</h3>                                        <p className="text-sm text-stone-600">{doc.docType}</p>                                    </div>                                </div>                            ))}                        </div>                    </div>                    <div className="lg:col-span-3 min-h-[50vh] lg:min-h-0 bg-stone-200/90 p-8 rounded-lg overflow-y-auto custom-scrollbar border-2 border-stone-400 shadow-inner" style={{backgroundImage: "url(https://www.transparenttextures.com/patterns/old-paper.png)"}}>                        {selectedDocument ? (                            <>                                <h2 className="text-4xl font-bold text-stone-900 mb-2" style={{fontFamily: "'Cormorant Garamond', serif"}}>{selectedDocument.title}</h2>                                <p className="text-lg text-stone-600 mb-4 font-semibold border-b-2 border-stone-400 pb-2">{selectedDocument.docType}</p>                                <div className="text-stone-800 whitespace-pre-wrap leading-relaxed text-lg" dangerouslySetInnerHTML={{ __html: selectedDocument.content.replace(/\u005B([^\u005D]+)\u005D:/g, '<strong>[$1]:</strong>') }}></div>                                                                {selectedDocument.detectiveHint && (                                     <div className="mt-6 p-4 bg-yellow-100/80 border-l-4 border-yellow-500 rounded-r-lg">                                        <h3 className="text-lg font-bold text-yellow-800 flex items-center"><Lightbulb className="w-5 h-5 mr-2"/> Dedektif İpucu</h3>                                        <p className="mt-1 text-yellow-900"><i>{selectedDocument.detectiveHint}</i></p>                                    </div>                                )}                                {selectedDocument.docType === "Çözüm" && (                                    <div className="mt-6 p-4 bg-amber-800/20 border-2 border-amber-700/50 rounded-lg">                                        <h3 className="text-2xl font-bold text-amber-900" style={{fontFamily: "'Cormorant Garamond', serif"}}>Gerçekler Ortaya Çıktı!</h3>                                        <p className="mt-2 text-stone-800"><strong>Suçlu:</strong> {gameData.culprit}</p>                                        <p className="mt-4 whitespace-pre-wrap text-stone-800"><strong>Nasıl Çözüldü:</strong> {gameData.solutionDetails}</p>                                    </div>                                )}                            </>                        ) : <div className="h-full flex items-center justify-center text-stone-500"><p>Bir belge seçin.</p></div>}                    </div>                </div>                <style>{`.custom-scrollbar::-webkit-scrollbar { width: 8px; } .custom-scrollbar::-webkit-scrollbar-track { background: transparent; } .custom-scrollbar::-webkit-scrollbar-thumb { background-color: #a8a29e; border-radius: 10px; }`}</style>            </div>        </>    );};export default function App() {    const [view, setView] = useState('config');    const [gameData, setGameData] = useState(null);    const [isLoading, setIsLoading] = useState(false);    const generateGameWithGemini = async (settings) => {        setIsLoading(true);        const prompt = `        Sen, "Usta Bir Dedektif Hikayesi Yazarı" rolünü üstlenen bir yapay zeka sistemisin. Görevin, sürükleyici anlatılar ve zekice ve kurnazca kurgulanmış bulmacalar yaratmaktır. Lütfen aşağıda belirtilen parametrelere göre, birbiriyle bağlantılı, zekice tasarlanmış ve oynamaya hazır bir dedektiflik macerası için tam bir dava dosyası oluştur. Tüm metinler Türkçe olmalıdır. Çıktın SADECE istenen JSON formatında olmalıdır.        Kullanıcı Parametreleri:        - Tema: ${settings.theme || 'Klasik Cinayet Gizemi'}        - Olay: ${settings.eventType || 'Zengin bir iş adamı malikanesinde ölü bulundu'}        - Mekan: ${settings.location || 'İngiliz kırsalında bir malikane'}        - Zorluk: ${settings.difficulty}        - Tahmini Oyun Süresi: ${settings.playTime}        - Şüpheli Sayısı: ${settings.suspectCount}                YAPILACAKLAR:        1.  **Ana Başlık ve Olay Örgüsü:** Oyun için çekici bir başlık ve olayın genel arka planını anlatan kısa bir ana hikaye oluştur.        2.  **Kilit Obje Yaratımı:** Hikayenin çözümünde önemli rol oynayacak, temayla uyumlu bir 'kilit obje' yarat ve bunu kanıtların içine entegre et.        3.  **Belge Üretimi:** Toplam belge sayısı en az (şüpheli sayısı + 4) olmalıdır. Her belgeye benzersiz bir 'docId' ata (1'den başlayarak).            -   **Sorgu Raporu:** Her şüpheli için ayrı bir rapor oluştur. 'content' kısmı, dedektif ve şüpheli arasında geçen **detaylı bir diyalog metni** olmalıdır. Diyalog, şüphelinin kişiliğini, yalanlarını ve potansiyel ipuçlarını yansıtmalıdır. Diyalog formatı şu şekilde olsun: "[Dedektif]: Soru metni...\n[Şüpheli Adı]: Cevap metni...".             -   **Dedektif İpucu:** Eğer zorluk seviyesi 'Kolay' veya 'Normal' ise, her sorgu raporu için oyuncuya yol gösterecek kısa, zekice bir 'detectiveHint' alanı ekle. Bu ipucu, şüphelinin ifadesindeki bir çelişkiye veya dikkat edilmesi gereken bir detaya işaret etmelidir. Eğer zorluk 'Zor' veya 'Uzman' ise, bu alanı boş bırak veya hiç ekleme.        4.  **Diğer Belgeler:** 'Gazete Küpürü', 'Otopsi Raporu', 'Olay Yeri Raporu', 'Kişisel Not' gibi çeşitli kanıt belgeleri oluştur.        5.  **Kilitli Dosyalar ve Şifreler:** Belgelerden en az iki tanesini kilitli ('locked': true) yap. Şifreler, diğer AÇIK belgelerin içinde bulunabilecek şekilde zekice gizlenmelidir.        6.  **Suçlu ve Çözüm:** Şüphelilerden BİRİNİ suçlu olarak belirle. Oyunun sonunda gösterilecek olan, suçlunun kim olduğunu, nedenini ve hangi ipuçlarının birleştirilerek bu sonuca ulaşıldığını detaylıca anlatan bir çözüm metni yaz. Bu çözüm için "Çözüm" adında ve docType'ı "Çözüm" olan özel bir belge oluştur. Bu belge başlangıçta HER ZAMAN kilitli olmalı ve şifresi ('password') zor olmalıdır (örneğin "DAVAYIÇÖZDÜM").        7.  **JSON Formatı:** Tüm bu çıktıları, aşağıda tanımlanan JSON şemasına %100 uygun olarak TEK BİR JSON nesnesi içinde döndür. JSON dışında hiçbir metin ekleme.        `;        const schema = { type: "OBJECT", properties: { "title": { "type": "STRING" }, "mainStory": { "type": "STRING" }, "culprit": { "type": "STRING" }, "solutionDetails": { "type": "STRING" }, "documents": { "type": "ARRAY", "items": { "type": "OBJECT", "properties": { "docId": { "type": "NUMBER" }, "docType": { "type": "STRING", "enum": ["Sorgu Raporu", "Gazete Küpürü", "Otopsi Raporu", "Olay Yeri Raporu", "Kişisel Not", "Harita", "Çözüm"] }, "title": { "type": "STRING" }, "locked": { "type": "BOOLEAN" }, "password": { "type": "STRING" }, "content": { "type": "STRING" }, "detectiveHint": {"type": "STRING"} }, "required": ["docId", "docType", "title", "locked", "content"] } } }, "required": ["title", "mainStory", "culprit", "solutionDetails", "documents"] };        try {            const apiKey = import.meta.env.VITE_GEMINI_API_KEY;             const apiUrl = `https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${apiKey}`;            const payload = { contents: [{ role: "user", parts: [{ text: prompt }] }], generationConfig: { responseMimeType: "application/json", responseSchema: schema } };            const response = await fetch(apiUrl, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });            if (!response.ok) throw new Error(`API hatası: ${response.status} ${response.statusText}`);            const result = await response.json();            if (result.candidates && result.candidates[0] && result.candidates[0].content.parts[0].text) {                const gameJson = JSON.parse(result.candidates[0].content.parts[0].text);                setGameData(gameJson);                setView('game');            } else { throw new Error("API'den geçerli bir oyun verisi alınamadı."); }        } catch (error) {            console.error("Oyun oluşturulurken hata oluştu:", error);            setView('config');        } finally {            setIsLoading(false);        }    };        const handleBackToConfig = () => {        setGameData(null);        setView('config');    };    switch (view) {        case 'loading': return <LoadingScreen />;        case 'game': return <GameScreen gameData={gameData} onBack={handleBackToConfig}/>;        case 'config': default: return <ConfigScreen onGenerate={generateGameWithGemini} isLoading={isLoading} />;    }}
    );
};
