...
const ConfigScreen = ({ onGenerate, isLoading }) => {
    ...
    const FormCategory = ({ title, children }) => (
        <div className="bg-cover p-6 rounded-lg shadow-lg" style={{ backgroundColor: '#fdf6e3' }}>
            <h2 className="text-2xl font-bold text-stone-800 mb-4 border-b-2 border-stone-400 pb-2 flex items-center">
    ...
    return (
        <div className="min-h-screen bg-stone-950 p-4 sm:p-8 flex items-center justify-center font-serif" style={{backgroundImage: "url(https://www.transparenttextures.com/patterns/dark-wood.png)"}}>
                ...
                <form onSubmit={(e) => { e.preventDefault(); onGenerate(settings); }}>
                    <div className="grid md:grid-cols-2 gap-8 mb-6">
                        <FormCategory title="Hikaye Temelleri">
                             <button type="button" onClick={handleRandomizeAll} disabled={isLoading || isRolling} className="flex items-center justify-center gap-2 w-full mb-4 px-4 py-2 bg-amber-500 border border-amber-600 rounded-lg text-white hover:bg-amber-400 transition-all duration-300 disabled:opacity-50 shadow-md">
...
const LoadingScreen = () => (
    <div className="min-h-screen bg-stone-950 flex flex-col items-center justify-center text-stone-200 font-serif" style={{backgroundImage: "url(https://www.transparenttextures.com/patterns/dark-wood.png)"}}>
...
const PasswordModal = ({ doc, onUnlock, onClose, setWrongPassword, wrongPassword }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm flex items-center justify-center z-50 transition-opacity duration-300">
            <div class={`bg-stone-800 p-8 rounded-lg shadow-2xl border-2 border-stone-600 w-full max-w-md text-center transform transition-all duration-300 ${wrongPassword ? 'animate-shake' : ''}`}>
